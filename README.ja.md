...
> **警告 — 使う前に必ず読んでください。** このツールは公式のX APIではなく、**非公式のブラウザCookieベースの自動化**（[twitter-cli](https://github.com/jackwener/twitter-cli)）でX（Twitter）の読み取り・投稿を行います。これはXの利用規約に違反する可能性があり、アカウントが**レート制限・凍結・永久BAN**される恐れがあります。本ツールは無保証で、教育・個人利用を目的に現状のまま提供されます。使用前に[免責事項](#免責事項)を必ず確認してください。

---

## 免責事項

本プロジェクトは公式のX APIを使用していません。[twitter-cli](https://github.com/jackwener/twitter-cli)をラップしており、twitter-cliは**ブラウザのセッションCookie**（`auth_token`・`ct0`）で認証し、ログイン済みブラウザと同じようにX（Twitter）へアクセスします。

**利用規約について。** 公式API以外の自動的な手段によるXへのアクセス（自動読み取り・スクレイピング・自動投稿）は、[Xの利用規約](https://x.com/ja/tos)、[X Developer Agreement and Policy](https://developer.x.com/en/developer-terms/agreement-and-policy)、[Xの自動化に関するルール](https://help.x.com/ja/rules-and-policies/x-automation)に違反する可能性があります。

**アカウントのリスク。** `--auto`・`generate-batch`・`engage`（自動いいね）・`recycle`・cronによる定期投稿といった自動・大量の操作は、Xのスパム対策・自動化ルールが制限している行為そのものです。これらの機能を使うとアカウントが**レート制限・凍結・永久BAN**される可能性があり、Xは予告なく措置を取ることがあります。また非公式のCookieベースの仕組みは、Xの内部仕様変更により予告なく動作しなくなる可能性があります。

**利用者の責任。**

- **自分自身が所有するアカウントでのみ**使用してください。管理権限のないアカウントには絶対に使用しないでください。
- **投稿された内容（AIが生成したテキストを含む）の責任はすべて利用者にあります。** 投稿前に必ず内容を確認し、完全自動モード（`--auto`・`chain`・cron）の利用には特に注意してください。
- スパム、大量フォロー、大量エンゲージメント、その他Xが禁止する行為に使用しないでください。
- 記事からツイートを生成する際は、引用元の著作権を尊重し、本文の丸写しを避け、要約と出典明記を行ってください。

**無保証。** 本ソフトウェアは「現状のまま」提供され、いかなる保証もありません。作者は、本ツールの使用により生じたアカウント凍結・データ消失・その他一切の損害について**責任を負いません**。**自己責任でご利用ください。** 規約に準拠した方法が必要な場合は、[公式のX API](https://developer.x.com/en/docs/x-api)を使用してください。
